...
import (
	"fmt"
	"github.com/yunnet/plugin-firefly/file"
	"os"
	"strings"
)

func BackupPath(path string) file.Option {
	return file.MakeOption("backupPath", path)
}

func (is *InterfaceSet) Write(opts ...file.Option) error {
	fnConfig := file.MakeConfig(
		file.Defaults{"path": "output"},
		opts,
	)
...
func copyFileIfExists(path, backupPath string) error {
	if _, err := os.Stat(path); err == nil {
		err2 := file.CopyFile(path, backupPath)
		if err2 != nil {
			return err
		}
	}
